...
##        if self.i==0:
##            print(ds)
#        self.broadcast('s'+ str(self.comr), [ds.reshape(n,),es.reshape(n,)])
#        shares = self.get_shares('s'+ str(self.comr))
#        self.comr+=1
#        if self.i==0:
#            print(n)
#            print(shares.reshape(2,2*n))
#        shares = shares.reshape(self.n, 2*n)
#        
#        DS = shares[:,:n]
#        ES = shares[:,n:]
...
#        X = X1+X2
#        print(np.shape(np.dot(b,u)))
        
#        self.broadcast(str(i), (x))
#        xs = self.get_shares(str(i))
#        print(rns.rec(xs))
#        
        X = X1 + np.dot(b, u).reshape(3,1)
...
#        IS = R+ np.dot(H,IS1)
        IS_inv = self.inv(IS)
#        K1 = self.scalVecProd(IS_inv, H.T)
        K1 = np.dot(H.T , IS_inv).reshape(3,1)

        K = self.MatVecProd(P, K1)
        X = x + (y-IM)*K #self.scalVecProd((y-IM), K)
...
threads = []
for k in range(m):
    threads.append(party(k,m, [As[k], Bs[k], Cs[k], Xs[k], Ps[k], Qs[k], Rs[k], Ks[k], us[k], meass[k]], com, ques[k],
